...
app.post("/api/update", async (req, res) => {
    ...
    if (!bIsValid) return res.status(400).json({
        error: `Missing cookies '${[req.cookies["AURORA_ID"] ? null : "C_ID", req.cookies["C_TOKEN"] ? null : "C_TOKEN"].filter(x => x != null).join(", ")}'.`,
        errorCode: "cloud.id.update.invalid_fields",
        statusCode: 400
    })

    if (tokens[req.cookies["C_ID"]] != req.cookies["C_TOKEN"]) return res.status(401).json({
        error: `Invalid auth token '${req.cookies["C_TOKEN"]}'.`,
        errorCode: "cloud.id.me.invalid_auth_token",
        statusCode: 401
    })
